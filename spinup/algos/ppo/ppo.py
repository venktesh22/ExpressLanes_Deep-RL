...
def exportTollProfile(tollProfile, logger_kwargs, outputVec):
    ...
    f.write("Corresponding revenue($)= %f, TSTT(hrs)= %f, Throughput(veh)=%f, JAHstat= %f, remaining vehicles= %f, JAHstat2=%f, RMSEML_vio=%f, percentTimeViolated(%%)=%f, tdJAHMax= %f" % 
          (outputVec[1],outputVec[2],outputVec[3],outputVec[4],outputVec[5],outputVec[6], outputVec[7], outputVec[8], outputVec[9]))
    f.close()
    
#    print(data.to_string())
#    
#    plt.figure()
#    plt.title("Toll Profile")
#    plt.xlabel("Time (sec)")
#    plt.ylabel("Toll ($)")
#    
#    
#    for name in colNames:
#        plt.plot(data['Timestep'], data[name], '-', color = 'darkred', label = name)
#    
#    plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='meme-v0')
    ...
    parser.add_argument('--exp_name', type=str, default='ppo') #I assume this is the name of the folder with inputs
...
#    logger_kwargs = setup_logger_kwargs(args.exp_name, args.seed)
    ...
    logger_kwargs = setup_logger_kwargs(args.exp_name, str(args.seed)+"_e"+str(args.epochs)+"_"+args.objective+"_"+str(currTime)+"_PPO")


    ppo(lambda : gym.make(args.env, netname=args.exp_name, objective=args.objective, seed=args.seed,  jahThresh= args.jahThresh), 
        actor_critic=core.mlp_actor_critic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, 
        seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
        logger_kwargs=logger_kwargs, clip_ratio = args.clp_ratio, pi_lr = args.pi_lr)