...
#                     output_activation=tf.tanh, #output_activation=None, 
#                     policy=mlp_gaussian_policy, action_space=None):
#    
#    # policy
#    with tf.variable_scope('pi'):
...
#        mu, pi, logp_pi = apply_squashing_func(mu, pi, logp_pi)
#
##    # make sure actions are in correct range -ORIG CODE
##    action_scale = action_space.high[0]
##    mu *= action_scale
##    pi *= action_scale
##    # keeping actions between -1 and 1 so I can do post processing later
#
#    # vfs
#    vf_mlp = lambda x : tf.squeeze(mlp(x, list(hidden_sizes)+[1], activation, None), axis=1)
#    with tf.variable_scope('q1'):
